...
describe('parseRunLimit', () => {
  // The cap is all that stands between a cron run and a lot of cold email.
  // Number("abc") is NaN, and `collected.length >= NaN` is always false, so an
  // unvalidated limit removed the cap rather than failing.
  it('falls back rather than removing the cap', () => {
    expect(parseRunLimit('abc', 12)).toBe(12);
    expect(parseRunLimit('', 12)).toBe(12);
    expect(parseRunLimit(undefined, 12)).toBe(12);
    expect(parseRunLimit(null, 12)).toBe(12);
    expect(parseRunLimit('0', 12)).toBe(12);
    expect(parseRunLimit('-5', 12)).toBe(12);
    expect(parseRunLimit('Infinity', 12)).toBe(12);
  });

  it('accepts a positive number and truncates it', () => {
    expect(parseRunLimit('8', 12)).toBe(8);
    expect(parseRunLimit('3.9', 12)).toBe(3);
    expect(parseRunLimit('1', 12)).toBe(1);
  });
});
...
describe('calculator model', () => {
  ...
  // Radio groups make this impossible in the form, but `?s=` is
  // hand-editable, shared, and generated by outreach. Counting two websites
  // would quote savings on a stack nobody has.
  it('keeps only the first pick from a single-select section', () => {
    expect(normalizeSelection(['wix', 'squarespace', 'godaddy'])).toEqual(['wix']);
    expect(normalizeSelection(['email_1_3', 'email_11_plus'])).toEqual(['email_1_3']);
  });

  it('leaves multi-select sections alone', () => {
    const both = ['managed_hosting', 'unused_saas'];
    expect(normalizeSelection(both)).toEqual(both);
  });

  it('allows one pick from each different single section', () => {
    expect(normalizeSelection(['wix', 'email_1_3'])).toEqual(['wix', 'email_1_3']);
  });

  it('still drops unknown ids and duplicates', () => {
    expect(normalizeSelection(['wix', 'nope', 'wix'])).toEqual(['wix']);
    expect(normalizeSelection([])).toEqual([]);
  });

  it('never overstates the total from a crafted url', () => {
    // Wix alone is $27 recoverable. Adding a second website must not stack.
    expect(estimateMonthlyWaste(['wix', 'squarespace', 'godaddy'])).toBe(27);
    expect(currentMonthlySpend(['wix', 'squarespace'])).toBe(29);
    expect(decodeSelection('wix,squarespace')).toEqual(['wix']);
    expect(encodeSelection(['wix', 'squarespace'])).toBe('wix');
    expect(buildPrefillMessage(['wix', 'squarespace'])).not.toContain('Squarespace');
  });
